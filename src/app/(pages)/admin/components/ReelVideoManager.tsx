...
const ReelVideoManager = () => {
  ...
  const [videos, setVideos] = useState<Video[]>([]);
    ...
    const fetchVideos=async()=>{
        try{
            const res = await videoService.fetchVideo(config.appwriteReelCollectionId);
            setVideos(
              res.map((video: Video) => ({
                id: video.id,
...
console.log(sanitizeFileName(videoFile.name));
    ...
    setStatus("");
    const video =await videoService.uploadVideo({videourl:videoUrl,videoname:sanitizeFileName(videoFile.name)},config.appwriteReelCollectionId)
   ...
   const handleDelete =async(videoId:string,videoname:string)=> {
    try {
      await videoService.deleteVideo(videoname,videoId,config.appwriteReelCollectionId);
  ...
  return (
      ...
      <h2 className="text-2xl font-bold">Reel Manager</h2>
...
export default ReelVideoManager;
